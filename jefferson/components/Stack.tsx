...
export const Stack = () => {
  return (
    <div className='flex flex-col justify-center items-center py-10 md:py-24 pb-20 md:pb-40 px-4'>
        ...
        <div className='w-full max-w-[1000px] bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8'>
            <div className='flex border-b border-gray-100 pb-4 md:pb-6 mb-6 md:mb-8'>
            ...
            <div className='flex flex-col gap-10'>
                {stackData.map((section, index) => (
                    <div key={index} className='flex flex-col md:flex-row items-start border-b border-dashed border-gray-200 last:border-0 pb-8 last:pb-0'>
                        ...
                        <div className='w-full md:w-3/4 flex flex-wrap gap-4'>
                            {section.items.map((item, i) => (
                                <span key={i} className={`px-5 py-2 rounded-xl text-sm font-bold tracking-wide hover:opacity-80 transition-all sub-head-text border ${item.className}`}>
                                    {item.name}
                                </span>
                            ))}
                        </div>
